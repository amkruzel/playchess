  ...
  async function searchForUser() {
    ...
    try {
      ...
      if (user) {
        ...
        const loggedInUserUpdate = await updateUser($currentUser.id, newUserData)
...
<form>
  <label for="friend">
    <input bind:value={username} type="search" id="friend" name="friend" placeholder="Search for friends">
  </label>
