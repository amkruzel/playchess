...
<dialog open>
  <article in:fly="{{y: -900, delay: 100, duration: 250}}" out:fly="{{y: -900}}">
    ...
    <a on:click|preventDefault={() => loginModalShouldBeVisible = !loginModalShouldBeVisible} 
       href="#"
       aria-label="Close" 
       class="close"></a>
    <form on:submit|preventDefault>
      <label for="username">Username
        ...
        <input placeholder="Username" 
                ...
                aria-invalid="{!username || username.length < 3 || username.length > 150}"
                bind:value={username} />
      </label>
      <label for="password">Password
        ...
        <input placeholder="Password" 
                ...
                minlength="8"
                maxlength="72"
                aria-invalid="{!password || password.length < 8 || password.length > 72}"
    ...
    <footer>
      <!-- svelte-ignore a11y-click-events-have-key-events -->
      <div role="button" class="secondary" on:click={signUp} aria-busy="{loadingSignUp}">Sign Up</div>
      <!-- svelte-ignore a11y-click-events-have-key-events -->
      <div role="button" on:click={loginHandler} aria-busy="{loadingLogin}">Login</div>
    </footer>
  </article>
</dialog>